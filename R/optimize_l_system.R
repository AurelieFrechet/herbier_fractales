...
draw_L_system <- function(axiom, rules, angle, depth) {
  for (i in 1:depth)
  ...
  points <- data.frame(actions, x1, y1, x2, y2, alfa = a, depth = d)
  points <- points[which(points$actions == "F"),]
  points
}

# Boom who's the faster now ??
# system.time(draw_L_system_fronkonstine(axiom = "F",
#                                        rules = list("F" = "F[+F]F[-F][F]"),
#                                        angle = 20,
#                                        depth = 5))
# system.time(draw_L_system(axiom = "F",
#                                        rules = list("F" = "F[+F]F[-F][F]"),
#                                        angle = 20,
#                                        depth = 5))
